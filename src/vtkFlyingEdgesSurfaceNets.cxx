...
vtkStandardNewMacro(vtkFlyingEdgesSurfaceNets);
...
struct NetsWorker
{
  ...
  template <typename TArray>
  void operator()(TArray* scalarsArray, vtkFlyingEdgesSurfaceNets* self, vtkImageData* input, int* updateExt,
...
struct TransformQuadsToTriangles
{
  ...
  void operator()(vtkIdType cellId, vtkIdType endCellId)
  {
    ...
    {
      if (this->TriStrategy != vtkFlyingEdgesSurfaceNets::TRIANGULATION_GREEDY)
      {
        points.GetTuple(quadConn[0], x0);
        points.GetTuple(quadConn[1], x1);
        points.GetTuple(quadConn[2], x2);
        points.GetTuple(quadConn[3], x3);
        if (this->TriStrategy == vtkFlyingEdgesSurfaceNets::TRIANGULATION_MIN_EDGE)
        {
          d02 = vtkMath::Distance2BetweenPoints(x0, x2) < vtkMath::Distance2BetweenPoints(x1, x3);
        }
        else // if (this->TriStrategy == vtkFlyingEdgesSurfaceNets::TRIANGULATION_MIN_AREA)
        {
      ...
      else // if (this->TriStrategy == vtkFlyingEdgesSurfaceNets::TRIANGULATION_GREEDY)
      {
        d02 = true;
      }
  ...
  int outputMeshType, vtkPolyData* output, vtkDataArray* newScalars, int triStrategy)
{
  ...
  outputMeshType =
    (outputMeshType == vtkFlyingEdgesSurfaceNets::MESH_TYPE_DEFAULT ? vtkFlyingEdgesSurfaceNets::MESH_TYPE_TRIANGLES
                                                           : outputMeshType);
  vtkLog(TRACE, "Transforming output mesh type to: " << outputMeshType);

  if (outputMeshType == vtkFlyingEdgesSurfaceNets::MESH_TYPE_QUADS)
  {
...
struct SelectWorker
{
  template <typename TArray>
  void operator()(
    TArray* newScalars, vtkPolyData* output, int outputStyle, vtkFlyingEdgesSurfaceNets* self, int cellSize)
  {
    ...
    if (outputStyle == vtkFlyingEdgesSurfaceNets::OUTPUT_STYLE_SELECTED)
    {
    ...
    vtkSMPTools::For(0, numCells,
      [&newScalars, outputStyle, &selectedCells, self, lMap](vtkIdType cellId, vtkIdType endCellId)
      {
        ...
        for (; cellId < endCellId; ++cellId)
        {
          ...
          if ((outputStyle == vtkFlyingEdgesSurfaceNets::OUTPUT_STYLE_BOUNDARY &&
                inTuple[1] == backgroundLabel) ||
            (outputStyle == vtkFlyingEdgesSurfaceNets::OUTPUT_STYLE_SELECTED &&
...
vtkFlyingEdgesSurfaceNets::vtkFlyingEdgesSurfaceNets()
{
...
vtkMTimeType vtkFlyingEdgesSurfaceNets::GetMTime()
{
...
void vtkFlyingEdgesSurfaceNets::InitializeSelectedLabelsList()
{
...
void vtkFlyingEdgesSurfaceNets::AddSelectedLabel(double label)
{
...
void vtkFlyingEdgesSurfaceNets::DeleteSelectedLabel(double label)
{
...
vtkIdType vtkFlyingEdgesSurfaceNets::GetNumberOfSelectedLabels()
{
...
double vtkFlyingEdgesSurfaceNets::GetSelectedLabel(vtkIdType ithLabel)
{
...
//
int vtkFlyingEdgesSurfaceNets::RequestData(vtkInformation* vtkNotUsed(request),
...
//------------------------------------------------------------------------------
bool vtkFlyingEdgesSurfaceNets::IsCacheEmpty()
{
  return (!this->StencilsCache || this->GeometryCache->GetNumberOfPoints() < 1);
}

//------------------------------------------------------------------------------
void vtkFlyingEdgesSurfaceNets::CacheData(vtkPolyData* pd, vtkCellArray* stencils)
{
...
//------------------------------------------------------------------------------
int vtkFlyingEdgesSurfaceNets::FillInputPortInformation(int, vtkInformation* info)
{
...
//------------------------------------------------------------------------------
void vtkFlyingEdgesSurfaceNets::PrintSelf(ostream& os, vtkIndent indent)
{
