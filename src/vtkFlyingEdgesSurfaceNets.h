...
/**
 * @class   vtkFlyingEdgesSurfaceNets
 * @brief   generate smoothed isocontours from segmented 3D image data (i.e., "label maps")
 *
 * vtkFlyingEdgesSurfaceNets creates boundary/isocontour surfaces from a label map
...
#ifndef vtkFlyingEdgesSurfaceNets_h
#define vtkFlyingEdgesSurfaceNets_h
...
class VTKFILTERSCORE_EXPORT VTK_MARSHALAUTO vtkFlyingEdgesSurfaceNets : public vtkPolyDataAlgorithm
{
public:
  ///@{
  /**
   ...
   * information.
   */
  static vtkFlyingEdgesSurfaceNets* New();
  vtkTypeMacro(vtkFlyingEdgesSurfaceNets, vtkPolyDataAlgorithm);
...
protected:
  vtkFlyingEdgesSurfaceNets();
  ~vtkFlyingEdgesSurfaceNets() override = default;
...
private:
  vtkFlyingEdgesSurfaceNets(const vtkFlyingEdgesSurfaceNets&) = delete;
  void operator=(const vtkFlyingEdgesSurfaceNets&) = delete;
};
